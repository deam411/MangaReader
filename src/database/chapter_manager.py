        ...
        try:
            ...
            for batch_start in range(0, total_pages, batch_size):
                ...
                with self.get_connection() as conn:
                    ...
                    # Prepara tutti i dati per executemany con dimensioni
                    batch_data = []
                    for i, page_data in enumerate(batch_pages):
                        ...
                        try:
                            from PyQt5.QtGui import QImage
                            temp_image = QImage()
                            if temp_image.loadFromData(page_data):
                                width = temp_image.width()
                                height = temp_image.height()
                        except Exception as e:
                            logger.warning(f"Could not extract dimensions for page {start_page_number + batch_start + i}: {e}")

                        batch_data.append((
                            chapter_id,
                            start_page_number + batch_start + i,
                            page_data,
                            width,
                            height
                        ))
