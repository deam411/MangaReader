    ...
    *args: tuple,
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for indx, item in enumerate(items):
            ...
            item_info = indx, item
            future = executor.submit(func, item_info, *args, task_info, chapter_titles)
            futures[future] = task
...
def download_chapter(
    ...
    if chapter_titles and indx_chapter < len(chapter_titles):
        chapter_name = chapter_titles[indx_chapter]
    else:
        chapter_name = f"Chapter {indx_chapter + 1}"

    download_path = create_download_directory(manga_name, chapter_name)
    num_pages = int(pages_per_chapter[indx_chapter])

    for page in range(1, num_pages + 1):
        success = attempt_download_page(page, base_download_link, str(download_path))
        if not success:
